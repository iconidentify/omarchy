...
# Fresh repair: probe/create/partial fail the leaf with a real status.
# list/backend on a new root are status 3 (preserve) or unreachable; the
# migration treats 3 as done so later migrations are not held hostage.
# stat skip is the ext4 fixture. timer/settings/limine run on the recursive repair.
for failure in probe create partial timer settings limine sudo; do
...
seed_old_marker
TEST_FILESYSTEM=ext2/ext3 run_migrate >/dev/null
...
! grep -q 'create-config' "${TEST_LOG:-/dev/null}" 2>/dev/null || fail 'non-btrfs does not create a Snapper config'
...
seed_old_marker
status=0
FAIL_AT=create run_migrate >"$FIXTURE/output" 2>&1 || status=$?
